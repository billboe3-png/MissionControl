...
def get_all_veeam_providers(db: Session) -> list[tuple[str, VeeamProvider]]:
    """Return list of (name, provider) for all enabled Veeam profiles and agent targets."""
    from app.repositories.integration_profile_repository import (
        IntegrationProfileRepository,
    )
    ...
    try:
        ...
        logger.debug("Failed to load Veeam profiles: %s", e)

    # 2. Agent targets with Veeam plugin enabled
    try:
        from app.models.db.agent import Agent
        from app.models.db.agent_remote_target import AgentRemoteTarget
        ...
        for target, agent in targets:
            tp = (target.target_plugins or "").lower()
            tags = (target.tags or "").lower()
            if "veeam" in tp or "veeam" in tags:
                try:
                    provider = _get_agent_provider(db, target.id)
                    result.append((f"{target.name} (Agent)", provider))
                except Exception as e:
                    try:
                        inv = _get_target_inventory(agent, target.id)
                        from .agent_provider import AgentVeeamProvider
                        def _stub_dispatch(cmd):
                            return {"success": False, "error": "Agent dispatch not ready"}
                        provider = AgentVeeamProvider(
                            inv.get("veeam", {}),
                            target_hostname=target.hostname,
                            agent_id=agent.id,
                            target_id=target.id,
                            dispatch_cmd=_stub_dispatch,
                        )
                        result.append((f"{target.name} (Agent)", provider))
                    except Exception as inner_e:
                        logger.debug("Failed to build fallback agent Veeam provider for target %s: %s", target.id, inner_e)
    except Exception as e:
        logger.debug("Could not load agent targets for Veeam providers: %s", e)
