"""
Mission Control WinRM Automation Provider

Executes playbook steps via WinRM connections using existing
...
class WinRMAutomationProvider(AutomationProvider):
    """Execute playbook steps via WinRM."""
    ...
    def supported_step_types(self) -> list[str]:
        return ["remote_command", "powershell"]
    ...
    async def execute_step(
        ...
        try:
            ...
            duration_ms = int((time.monotonic() - start) * 1000)
            logger.error(
                "WinRM automation step failed: %s", exc
            )
            return StepResult(
                success=False,
                error=str(exc),
