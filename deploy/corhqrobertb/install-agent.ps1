...
# Copy agent package
Copy-Item -Recurse -Force "$PackageRoot\*" "$InstallDir\agent\" -ErrorAction SilentlyContinue
if (-not (Test-Path "$InstallDir\agent\__main__.py")) {
    Write-Fail "Agent package files were not copied to $InstallDir\agent"
    exit 1
}
